...
class LinksHandler(object):
    '''
    This plugin facilitates creating
    a list of resources you want to
    save while using Zulip. It looks for
    messages starting with "@link" or
    "@resource". In this example,we send
    resources to private messages
    '''

    def usage(self):
        return '''
            This plugin will allow users to flag
            messages as being resources and store
            them in private messages with the bot.
            Users should preface messages with
            "@link" or "@resource"
            '''
